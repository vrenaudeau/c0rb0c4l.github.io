...
function generateGrid(id) {
    ...
    if (!isGridGenerated) {
        ...
        score = 0;
        document.getElementById('js-score-size').insertAdjacentText("beforeend", ' (' + id + 'x' + id + ')');
...
function isWon() {
    ...
    if (count === size * size) {
        document.getElementById('js-display-score').insertAdjacentText("afterend", ' VICTOIRE !!');
